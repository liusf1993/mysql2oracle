...
def convert_index_ddl(table_name):
    ...
    for line in ddl.split("\n"):
        ...
        if key_match:
            ...
            key_props = pkey_match[1].replace("`", "")
            i_ddl += "alter table " + table_name + " add constraint P_" + table_name[2:] \
                     + " primary key (" + key_props + ");\n"
    f = open("out/%s.tab" % table_name, "a")
    f.write(i_ddl)
    f.close()
